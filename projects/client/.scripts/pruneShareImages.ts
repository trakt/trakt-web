 ...
 * interval for its `releasedAt` age, so media inside its churn window
 * gets regenerated frequently while settled media is evicted rarely.
 *
 * `releasedAt` is written by `buildImageMetadata`: the release date for
 * movies, the last aired episode for shows. So a long running show that
 * is still airing keeps landing in the aggressive tiers instead of
 * decaying from its premiere.
 */
...
type RecheckSchedule = {
  tiers: ReadonlyArray<RecheckTier>;
  ...
  for (const prefix of prefixes) {
    ...
    do {
      ...
      for (const object of listed.objects) {
        ...
        // Legacy entries (pre-releasedAt metadata) are always pruned so
