...
describe('util: getWellKnownSourceGroup', () => {
  it('should map amazon variants to a single brand group', () => {
    expect(getWellKnownSourceGroup('amazon_prime_video')).toBe(
      getWellKnownSourceGroup('amazon_prime_video_with_ads'),
    );
  });

  it('should not group sources that are not a known slug', () => {
    expect(getWellKnownSourceGroup('amazon_video')).toBeUndefined();
  });
