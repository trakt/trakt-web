<script lang="ts">
  ...
  import type { NoveltySwitchProps } from "./NoveltySwitchProps";
  ...
  const {
    ...
    indeterminate,
    ...props
  }: NoveltySwitchProps = $props();
</script>

<!--
  The toggle with a face. Kept deliberately as it was - gradient tick, icon in
  the thumb, a word inside the track - because the seasonal filters and the
  design system's theme toggle are FLAIR, and flair is the one place the flat
  settings switch would be the wrong answer.

  Nothing in settings may reach for this. See Switch.svelte.
-->
<label class="trakt-novelty-switch" class:has-custom-icon={!!icon}>
  <input
    type="checkbox"
    role="switch"
    data-color={color}
  ...
  <span class="trakt-novelty-switch-tick">
    {#if icon}
      {@render icon()}
    {:else}
      <SwitchIcon />
  ...
  {#if innerText && !indeterminate}
    <span class="trakt-novelty-switch-text bold ellipsis">
      {innerText}
    </span>
  {/if}
</label>
...
<style lang="scss">
  ...
  .trakt-novelty-switch.has-custom-icon {
    &:has(input:checked) {
      .trakt-novelty-switch-tick {
        :global(svg) {
