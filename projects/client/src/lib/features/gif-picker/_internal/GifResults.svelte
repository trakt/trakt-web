<script lang="ts">
  ...
  const gifs = results.pipe(
    map((state) => dedupe((gif) => gif.id, flattenQueryPages(state))),
  );
  const pageCount = results.pipe(
    map((state) => state.data?.pages.length ?? 0),
  );
...
<div class="trakt-gif-results">
  {#if $gifs.length > 0}
    ...
    {#if $hasNextPage}
      <!-- The observer fires once, so the sentinel is rebuilt per page. -->
      {#key $pageCount}
