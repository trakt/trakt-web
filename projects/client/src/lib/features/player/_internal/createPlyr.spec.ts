...
/**
 * `createPlyr` memoizes the script load at module scope, so each case needs a
 * fresh module instance to exercise a first attempt.
 */
...
describe('util: createPlyr', () => {
  ...
  it('should inject the script on first use', async () => {
    ...
    void createPlyr(document.createElement('div'), {}).catch(() => {});
