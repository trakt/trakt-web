...
export function usePopupHelpers(placement?: PopupPlacement) {
  ...
  const removeCloneAfterContainer = (popupContainer: HTMLElement) => {
    const currentClone = targetClone.value;
    if (currentClone) {
      currentClone.style.setProperty('pointer-events', 'none');
      currentClone.setAttribute(POPUP_STATE_ATTRIBUTE, PopupState.Removing);
      targetClone.next(currentClone);
    }
