...
export function useMarkAsWatched(
  ...
  const removeWatched = async () => {
    ...
    // Undo can only restore what the snapshot captured. A show-wide removal
    // has no per-episode dates to restore and re-adding the show would mark
    // episodes that were never watched, so it gets no undo at all.
