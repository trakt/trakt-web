<script lang="ts">
  ...
  import { StreamingServiceLogoIntlProvider } from "$lib/components/media/streaming-service/StreamingServiceLogoIntlProvider.ts";

  const { source }: { source: string } = $props();
</script>

<span class="trakt-streaming-service-select-logo" aria-hidden="true">
...
<style lang="scss">
  .trakt-streaming-service-select-logo {
    display: flex;
    align-items: center;
    ...
    :global(.trakt-streaming-service-logo.has-logo span) {
      display: none;
    }
