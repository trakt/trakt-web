...
const imdbTitleTypeMap: Record<string, UniversalImportItem['type']> = {
  movie: 'movie',
  tvmovie: 'movie',
  tvseries: 'show',
  tvminiseries: 'show',
...
function toImportType(titleType?: string): UniversalImportItem['type'] {
  ...
  return imdbTitleTypeMap[normalized] ?? 'movie';
}
...
export const ImdbParser: FileParser = {
  ...
  canParse(files) {
    return files.every((f) => f.name.toLowerCase().endsWith('.csv'));
  },
