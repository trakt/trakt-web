import type { TranslationResponse } from '@trakt/api';

export const ShowSiloLanguageResponseMock: TranslationResponse = [
  {
    'title': null,
    'overview':
      'في مستقبل خرب وسامّ، يعيش آلاف الناس في صومعة ضخمة تحت الأرض. بعد أن يخرق مأمورهم قانونها الأساسي ويموت المواطنون بطرق غامضة، تبدأ المهندسة "جولييت" بكشف أسرار صادمة لتعرف الحقيقة عن الصومعة.',
    'tagline': 'تحتاج إلى هدف كبير لتحيا من أجله.',
    'language': 'ar',
    'country': 'sa',
  },
  {
    'title': null,
    'overview':
      'في مستقبل خرب وسامّ، يعيش آلاف الناس في صومعة ضخمة تحت الأرض. بعد أن يخرق مأمورهم قانونها الأساسي ويموت المواطنون بطرق غامضة، تبدأ المهندسة "جولييت" بكشف أسرار صادمة لتعرف الحقيقة عن الصومعة.',
    'tagline': 'تحتاج إلى هدف كبير لتحيا من أجله.',
    'language': 'ar',
    'country': 'ae',
  },
  {
    'title': 'Силоз',
    'overview':
      'Мъжете и жените живеят в гигантски бункер под земята с множество правила, които според тях трябва да ги предпазят от токсичния и опустошен свят на повърхността.',
    'tagline': 'Истината ще излезе на повърхността.',
    'language': 'bg',
    'country': 'bg',
  },
  {
    'title': null,
    'overview':
      'En un futur ruïnós i tòxic, hi ha una comunitat en una gegantina sitja subterrània que arriba a centenars de pisos de profunditat. Allí, les persones viuen en una societat plena de normes que creuen que estan destinades a protegir-les.',
    'tagline': 'La veritat sortirà a la superfície.',
    'language': 'ca',
    'country': 'es',
  },
  {
    'title': null,
    'overview':
      'Ve zmařené, toxické budoucnosti žijí tisíce lidí v obřím silu hluboko pod zemí. Poté, co místní šerif poruší základní pravidlo a obyvatelé záhadně umírají, začne mechanička Juliette odhalovat šokující tajemství a pravdu o sile.',
    'tagline': 'Abyste mohli žít, musíte mít pro co zemřít.',
    'language': 'cs',
    'country': 'cz',
  },
  {
    'title': null,
    'overview':
      'I en hærget, giftig fremtid lever tusinder i en kæmpemæssig underjordisk silo. Efter at sheriffen bryder en ufravigelig regel, og beboere dør på mystisk vis, begynder teknikeren Juliette at afdække chokerende hemmeligheder og sandheden om siloen.',
    'tagline': 'Man lever ikke, før man har noget at dø for.',
    'language': 'da',
    'country': 'dk',
  },
  {
    'title': null,
    'overview':
      'In einer verwüsteten und giftigen Zukunft leben Tausende Menschen in einem gewaltigen Silo tief unter der Erde. Nachdem der Sheriff eine zentrale Regel bricht und mysteriöse Todesfälle unter den Bewohnern auftreten, macht sich die Ingenieurin Juliette daran, die schockierenden Geheimnisse über die wahre Natur des Silos zu enthüllen.',
    'tagline': 'Jedes Leben braucht etwas, für das es sich zu sterben lohnt.',
    'language': 'de',
    'country': 'de',
  },
  {
    'title': 'Σιλό',
    'overview':
      'Σε ένα ερειπωμένο και τοξικό μέλλον, χιλιάδες άνθρωποι ζουν σε ένα γιγάντιο σιλό βαθιά κάτω από τη γη. Αφού ο σερίφης του παραβιάζει έναν θεμελιώδη κανόνα και οι κάτοικοι πεθαίνουν μυστηριωδώς, η μηχανικός Τζούλιετ αρχίζει να ανακαλύπτει συγκλονιστικά μυστικά και την αλήθεια για το σιλό.',
    'tagline':
      'Για να ζήσεις, χρειάζεσαι κάτι για το οποίο αξίζει να πεθάνεις.',
    'language': 'el',
    'country': 'gr',
  },
  {
    'title': 'Silo',
    'overview':
      'In a ruined and toxic future, thousands live in a giant silo deep underground. After its sheriff breaks a cardinal rule and residents die mysteriously, engineer Juliette starts to uncover shocking secrets and the truth about the silo.',
    'tagline': 'The truth will surface.',
    'language': 'en',
    'country': 'us',
  },
  {
    'title': null,
    'overview':
      'En un futuro ruinoso y tóxico, existe una comunidad en un gigantesco silo subterráneo que alcanza cientos de pisos de profundidad. Allí, las personas viven en una sociedad llena de normas que creen que están destinadas a protegerlas.',
    'tagline': 'Para sobrevivir, necesitas algo por lo que morir.',
    'language': 'es',
    'country': 'es',
  },
  {
    'title': null,
    'overview':
      'En un futuro arruinado y tóxico, existe una comunidad en un enorme silo subterráneo cientos de pisos bajo tierra. Ahí, hombres y mujeres viven en una sociedad llena de reglas que creen que están hechas para protegerlos.',
    'tagline': 'Para vivir, necesitas algo por qué morir.',
    'language': 'es',
    'country': 'mx',
  },
  {
    'title': 'سیلو',
    'overview':
      'در آینده‌ای ویران‌شده و سمی، جامعه‌ای در یک سیلو زیرزمینی غول‌پیکر وجود دارد که صدها طبقه را در اعماق فرو می‌برد. در آنجا، مردان و زنان در جامعه ای پر از مقررات زندگی می کنند که معتقدند برای محافظت از آنها طراحی شده است.',
    'tagline': null,
    'language': 'fa',
    'country': 'ir',
  },
  {
    'title': 'Siilo',
    'overview':
      'Tulevaisuus on tuhoutunut ja myrkyllinen, ja tuhannet ihmiset elävät jättimäisessä maanalaisessa siilossa. Kun siilon seriffi rikkoo tärkeää sääntöä ja kansalaiset alkavat kuolla salaperäisesti, Juliette alkaa selvittää järkyttäviä salaisuuksia ja totuutta siilosta.',
    'tagline': 'Selviytyminen vaatii uhrauksia.',
    'language': 'fi',
    'country': 'fi',
  },
  {
    'title': null,
    'overview':
      "Dans un monde futur dévasté et toxique, des milliers de survivants habitent un gigantesque silo souterrain. Après la transgression d'une règle d'or par leur shérif et la mort mystérieuse de plusieurs résidents, Juliette, une mécanicienne, perce peu à peu les terribles secrets du Silo.",
    'tagline':
      'Pour vivre, il faut quelque chose qui vaille la peine de mourir.',
    'language': 'fr',
    'country': 'fr',
  },
  {
    'title': null,
    'overview':
      "Dans un monde futur dévasté et toxique, des milliers de survivants habitent un gigantesque silo souterrain. Après la transgression d'une règle d'or par leur shérif et la mort mystérieuse de plusieurs résidents, Juliette, une mécanicienne, perce peu à peu les terribles secrets du Silo.",
    'tagline': 'Pour vivre, il faut avoir une raison de mourir.',
    'language': 'fr',
    'country': 'ca',
  },
  {
    'title': null,
    'overview':
      'בעתיד פוסט־אפוקליפטי ורעיל, אלפי אנשים מתגוררים בסילו תת־קרקעי עצום. לאחר שהשריף מפר חוק יסוד ותושבים מתים במסתוריות, ג׳ולייט, מהנדסת בסילו, מתחילה לחשוף סודות מטרידים ואת האמת שמאחורי הסילו.',
    'tagline': 'כדי לחיות, צריך משהו ששווה למות למענו.',
    'language': 'he',
    'country': 'il',
  },
  {
    'title': 'साइलो',
    'overview':
      'एक तहस-नहस हो चुके और ज़हरीले भविष्य में, हज़ारों लोग एक विशाल भूमिगत साइलो में रहते हैं। जब वहाँ के शेरिफ़ एक मुख्य नियम तोड़ते हैं और निवासी रहस्यमय तरीके से मरते हैं, इंजीनियर जूलिएट साइलो के बारे में चौंकाने वाले रहस्य और सच्चाई उजागर करना शुरू करती है।',
    'tagline': 'जीने के लिए कुछ ऐसा चाहिए जिसके लिए मरना भी गवारा हो।',
    'language': 'hi',
    'country': 'in',
  },
  {
    'title': 'A siló',
    'overview':
      'A jövő mérgező és lepusztult világában ezrek élnek egy hatalmas, föld alatti silóban. Miután a seriffjük megszeg egy rendkívül fontos szabályt, majd több lakó is rejtélyes halált hal, Juliette, a gépész nyomozásba kezd — ez megdöbbentő titkokról és igazságokról rántja le a leplet.',
    'tagline':
      'Ahhoz, hogy élni tudj, szükséged van valamire, amiért meghalnál.',
    'language': 'hu',
    'country': 'hu',
  },
  {
    'title': null,
    'overview':
      'Dalam masa depan yang hancur dan beracun, ribuan jiwa hidup dalam silo raksasa jauh di bawah tanah. Setelah sheriffnya melanggar aturan utama dan warga tewas secara misterius, insinyur Juliette mulai menguak rahasia yang mengejutkan dan kebenaran tentang silo.',
    'tagline': 'Untuk hidup, Anda butuh sesuatu yang pantas diperjuangkan.',
    'language': 'id',
    'country': 'id',
  },
  {
    'title': null,
    'overview':
      'In un futuro tossico e in rovina, migliaia di persone vivono in un enorme silo sotterraneo. Dopo la violazione di una regola cardine da parte dello sceriffo e la morte misteriosa di alcuni residenti, Juliette, un tecnico, comincia a svelare dei segreti scioccanti e la verità sul silo.',
    'tagline': 'Per vivere, serve qualcosa per cui valga la pena morire.',
    'language': 'it',
    'country': 'it',
  },
  {
    'title': 'サイロ',
    'overview':
      '有毒物質が蔓延する荒廃した未来。何千人もの人間が地下深くに広がる巨大なサイロで暮らしている。保安官が基本的な規則を破り、住民たちが謎の死を遂げる中、機械工のジュリエットはサイロの驚くべき秘密と真実を解き明かしていく。',
    'tagline': '生きるために、命をかけて追い求める。',
    'language': 'ja',
    'country': 'jp',
  },
  {
    'title': 'სილოსის ორმო',
    'overview': null,
    'tagline': null,
    'language': 'ka',
    'country': 'ge',
  },
  {
    'title': "'지하창고 사일로의 비밀' - Silo",
    'overview':
      '황폐해지고 유독한 물질로 가득한 미래, 수천 명의 사람이 지하 깊은 곳 거대한 사일로에서 살고 있다. 보안관이 사일로의 기본 규칙을 깨고 거주민들이 미심쩍은 죽음을 맞자, 엔지니어 줄리엣은 사일로에 관한 충격적인 비밀과 진실을 밝히기 시작한다.',
    'tagline': '살아남으려면, 목숨 걸고 지켜야 한다.',
    'language': 'ko',
    'country': 'kr',
  },
  {
    'title': 'Bunkeris',
    'overview':
      'Vyrai ir moterys gyvena milžiniškame silose po žeme, kur, jų manymu, taikomos kelios taisyklės, kurios apsaugo juos nuo toksiško ir sugriauto pasaulio.',
    'tagline': null,
    'language': 'lt',
    'country': 'lt',
  },
  {
    'title': null,
    'overview':
      'Siloဆိုတာကတော့ ၁၄၄ ထပ်ရှိတဲ့ မြေအောက်မျှော်စင် မြို့တော်ကြီးတစ်ခုပေါ့။ ပြင်ပကမ္ဘာကြီးကတော့ ကပ်ဆိုက်ပျက်စီးပြီးတော့ အဆိပ်သင့်နယ်မြေလို့ ယူဆကြပြီးတော့ ဒီSiloထဲက လူတစ်စုကတော့ Siloရဲ့ပြင်ပကိုထွက်ချင်ပါတယ်လို့ လုံးဝမပြောပါဘူးဆိုတဲ့ အဓိကဥပ‌ဒေတစ်ခုအပါအဝင် တခြားမြောက်များစွာသောစည်းကမ်းတွေကို မဖြစ်မနေလိုက်နာရပါတယ်။ ဒီမှာမေးစရာရှိလာတာက စည်းကမ်းမလိုက်နာရင် Siloရင်ပြင်ပကိုထွက်ချင်ပါတယ်လို့ ပြောလာတဲ့လူအတွက် ဘာပြစ်ဒဏ်ရှိလဲ။ ဒီလို အပြင်ထွက်ချင်တယ်လို့ ပြောလာသူတိုင်းကို အပြင်ထွက်ခွင့်ပေးပါတယ်။ အလဲအလှယ်အနေနဲ့ Siloရဲ့အပြင်ဘက်မှာတပ်ဆင်ထားတဲ့ ပြင်ပအာရုံခံစက်တွေကို သန့်ရှင်းရေးလုပ်ရပါတယ်။ ဒါပေမဲ့ထူးခြားတာကတော့ အပြင်ကိုထွက်ချင်တယ်ပြောမိသူတိုင်း မဖြစ်မနေ သန့်ရှင်းရေးလုပ်စရာမလိုပေမဲ့ သန့်ရှင်းရေးလုပ်ပြီးသူတိုင်းတော့ မိနစ်ပိုင်းအတွင်း သေဆုံးကုန်ကြပါတယ်။ ရှေ့ကလူတွေရဲ့ သေဆုံးခြင်းကိုပဲ မျက်ဝါးထင်ထင် မြင်နေရချိန်မှာတော့ Siloမြို့ကပြည်သူတွေအနေနဲ့ အထွန့်မတက်ရဲတော့ဘဲ စည်းမျဉ်းစည်းကမ်းတွေများလွန်းတဲ့ ဒီမြို့မှာပဲ အသက်ရှင်ဖို့ကိုသာ အဓိကထားနေထိုင်ကြရင်း နှစ် ၁၄၀ ကျော်ကြာခဲ့ပါတယ်။',
    'tagline': null,
    'language': 'my',
    'country': 'my',
  },
  {
    'title': null,
    'overview':
      'In een verwoeste, vergiftigde toekomst leven duizenden mensen in een enorme silo onder de grond. Nadat de sheriff een belangrijke regel overtreedt en inwoners onverklaard overlijden, ontrafelt de technicus Juliette schokkende geheimen en de waarheid over de silo.',
    'tagline': 'Om te leven heb je iets nodig om voor te sterven.',
    'language': 'nl',
    'country': 'nl',
  },
  {
    'title': null,
    'overview':
      'I en ødelagt og giftig framtid bor tusenvis i en gigantisk silo dypt under jorden. Etter at sheriffen bryter en av de største reglene og beboere dør på mystisk vis, begynner ingeniøren Juliette å avdekke sjokkerende hemmeligheter og sannheten om siloen.',
    'tagline': 'For å leve må du være villig til å dø for noe.',
    'language': 'no',
    'country': 'no',
  },
  {
    'title': 'Silos',
    'overview':
      'W zniszczonym, toksycznym świecie przyszłości tysiące ludzi żyją w ogromnym podziemnym silosie. Kiedy jego szeryf łamie podstawową zasadę, a mieszkańcy giną w tajemniczych okolicznościach, mechaniczka Juliette zaczyna odkrywać szokujące sekrety i prawdę na temat tego miejsca.',
    'tagline': 'By żyć, przygotuj się za to umrzeć.',
    'language': 'pl',
    'country': 'pl',
  },
  {
    'title': null,
    'overview':
      'Em um futuro destruído e tóxico, existe uma comunidade que vive dentro de um gigantesco silo subterrâneo com centenas de andares de profundidade. Lá, homens e mulheres vivem numa sociedade cheia de regras que acreditam existir para protegê-los.',
    'tagline': 'Para viver, você precisa de algo pelo qual morreria.',
    'language': 'pt',
    'country': 'br',
  },
  {
    'title': 'O Silo',
    'overview':
      'Num mundo futuro tóxico e em ruínas, milhares de pessoas vivem num gigantesco silo subterrâneo. Após a quebra de uma regra fundamental pelo xerife e a misteriosa morte de alguns residentes, a engenheira Juliette Nichols começa a desvendar segredos chocantes e a verdade sobre o silo.',
    'tagline': 'Para viver é preciso algo pelo qual morrer.',
    'language': 'pt',
    'country': 'pt',
  },
  {
    'title': 'Silozul',
    'overview':
      'Silozul este o construcţie subterană gigantică unde se înghesuie, pe 144 de niveluri, ultimii 10.000 de supravieţuitori ai rasei umane, după ce iresponsabilitatea generaţiilor anterioare a făcut Pământul nelocuibil. Imaginea dezastrului de la suprafaţă este încontinuu proiectată pe un ecran gigantic din cantina silozului, reamintindu-le rezidenţilor că regulile stricte care le guvernează societatea sunt singurele care îi protejează de o moarte brutală. După ce o răzmeriţă din trecut a dus la ştergerea tuturor înregistrărilor istorice, în prezent nimeni nu ştie cine şi când a construit silozul.',
    'tagline': null,
    'language': 'ro',
    'country': 'ro',
  },
  {
    'title': 'Укрытие',
    'overview':
      'На отравленной гибнущей планете будущего тысячи людей ютятся в огромном подземном укрытии. Нарушение шерифом основного правила и серия загадочных смертей вынуждают инженера Джульетту заняться раскрытием тайн и поиском правды о бункере.',
    'tagline': 'В жизни нужна цель, во имя которой можно умереть.',
    'language': 'ru',
    'country': 'ru',
  },
  {
    'title': null,
    'overview':
      'V zničenej a toxickej budúcnosti žijú tisíce ľudí v obrovskom sile hlboko pod zemou. Po tom, čo jeho šerif poruší základné pravidlo a obyvatelia záhadne umierajú, strojníčka Juliette začne odhaľovať šokujúce tajomstvá a pravdu o sile.',
    'tagline': 'Ak chcete žiť, potrebujete mať niečo, prečo zomrieť.',
    'language': 'sk',
    'country': 'sk',
  },
  {
    'title': 'Silos',
    'overview':
      'Zgodba sledi človeštvu, ki se je v prihodnosti preselilo v ogromen silos, ta pa pod zemljo ima kar 144 nadstropij. Na površju se namreč ne da živeti več, saj je površina radioaktivna, od te apokalipse pa so minile generacije, tako da se v bistvu nihče več niti ne spominja, kaj je bil sploh razlog za vso to radioaktivnost. O razlogih se sicer ne sme javno preveč razpravljati, saj če začneš dvomiti v razloge za življenje v Silosu, si lahko hitro obsojen na čiščenje zunanjih kamer, ki so še edini vpogled v zunanji svet, to pa je za čistilca tudi usodno. Ali pa morda ne? Po Silosu se namreč širijo govorice, da tiste kamere v resnici ne kažejo realne slike in da je površina povsem normalna, nekdo pa želi ljudi zaradi svoji razlogov zadržati v Silosu...',
    'tagline': 'Resnica bo prišla na dan.',
    'language': 'sl',
    'country': 'si',
  },
  {
    'title': 'Силос',
    'overview':
      'У уништеној и токсичној будућности, заједница постоји у огромном подземном силосу који урања стотине прича дубоко. Тамо мушкарци и жене живе у друштву пуном прописа за које верују да треба да их заштите.',
    'tagline': 'Истина ће испливати на површину.',
    'language': 'sr',
    'country': 'rs',
  },
  {
    'title': null,
    'overview':
      'I en framtid av ruiner och giftig miljö lever tusentals i en jättestor silo under jorden. När dess sheriff bryter mot en kardinalregel och dör mystiskt, börjar teknikern Juliette avslöja chockerande hemligheter och sanningen om silon.',
    'tagline': 'För att överleva behöver du något värt att dö för.',
    'language': 'sv',
    'country': 'se',
  },
  {
    'title': null,
    'overview':
      'ในอนาคตที่ล่มสลายและเป็นพิษ คนนับพันนับหมื่นชีวิตต้องอาศัยอยู่ในหลุมใต้ดินขนาดยักษ์ หลังจากที่นายอำเภอละเมิดกฎหลักของที่นั่นและมีประชาชนตายอย่างมีเงื่อนงำ วิศวกรที่ชื่อจูเลียตต์ ได้เริ่มค้นพบความลับอันน่าตกใจและความจริงเกี่ยวกับหลุมใต้ดินนี้',
    'tagline': 'หากจะมีชีวิตต่อ คุณก็ต้องมีสิ่งที่ยอมตายเพื่อให้ได้มา',
    'language': 'th',
    'country': 'th',
  },
  {
    'title': null,
    'overview':
      "Mahvolmuş ve zehirli bir gelecekte, binlerce kişi yeraltının derinliklerindeki dev bir siloda yaşamaktadır. Silo'nun şerifi temel bir kuralı çiğnedikten ve sakinler gizemli bir şekilde ölmeye başladıktan sonra mühendis Juliette, Silo hakkındaki şok edici sırları ve gerçeği ortaya çıkarmaya başlar.",
    'tagline': 'Gerçek ortaya çıkacaktır.',
    'language': 'tr',
    'country': 'tr',
  },
  {
    'title': 'Таємниця бункера',
    'overview':
      'У занапащеному й небезпечному майбутньому тисячі людей живуть у великому підземному бункері. Коли шериф порушує його основоположне правило, а жителі гинуть за загадкових обставин, інженер Джулієт береться розкривати приголомшливі таємниці й правду про бункер.',
    'tagline': 'У житті має бути щось, за що можна померти.',
    'language': 'uk',
    'country': 'ua',
  },
  {
    'title': null,
    'overview':
      'Trong thế giới hoang tàn và nhiễm độc ở tương lai, hàng nghìn người sống trong một silo khổng lồ sâu dưới lòng đất. Sau khi một cảnh sát trưởng phá vỡ một quy tắc chủ chốt và dân thường thiệt mạng một cách bí ẩn, kỹ sư Juliette bắt đầu phát hiện ra những bí mật gây sốc và sự thật về silo.',
    'tagline': 'Để sống sót, bạn phải đấu tranh đến cùng vì sự thật.',
    'language': 'vi',
    'country': 'vn',
  },
  {
    'title': '末日地堡',
    'overview':
      '在一個滿目瘡痍，佈滿有毒物質的未來世界中，成千上萬的人在一個地底的巨型地堡生活。當保安官違反了一個基本原則，而地堡出現居民神秘死亡事件之後，工程師茱麗葉開始發現地堡的駭人秘密和真相。',
    'tagline': '為了活下去，有人不惜以死相許。',
    'language': 'zh',
    'country': 'hk',
  },
  {
    'title': '末日地堡',
    'overview':
      '该剧基于休·豪伊所著同名畅销科幻系列小说改编，在一个满目疮痍、有毒物质遍布的未来世界中，成千上万的人居住在深埋在地下的巨大筒仓里。在警长违反了一项基本规则以及有些居民神秘死亡后，工程师朱丽叶开始揭开惊人的秘密和筒仓的真相。',
    'tagline': null,
    'language': 'zh',
    'country': 'cn',
  },
  {
    'title': null,
    'overview':
      '在一个满目疮痍、有毒物质遍布的未来世界中，成千上万的人居住在深埋在地下的巨大筒仓里。警长违反了一条基本规则，有些居民神秘死亡后，工程师朱丽叶开始揭开惊人的秘密和筒仓的真相。',
    'tagline': null,
    'language': 'zh',
    'country': 'sg',
  },
  {
    'title': '末日地堡',
    'overview':
      '在一個破敗和充滿毒害的未來，成千上萬人居住在一座地底深處的巨大地堡之中。在地堡的保安官違反重要規定，且有居民神祕死亡後，工程師茱麗葉開始揭發驚人祕密與地堡的真相。',
    'tagline': '擁有值得付出生命的事物，就能找到活下去的力量。',
    'language': 'zh',
    'country': 'tw',
  },
];
